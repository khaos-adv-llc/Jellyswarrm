...
public enum SeerrAuthMode: String, Codable, Sendable, CaseIterable {
    ...
    public var displayName: String {
        switch self {
        case .apiKey: "API Key"
        case .jellyfinCredentials: "Jellyfin Account"
        case .localAccount: "Local Seerr Account"
        }
    }
    ...
    public var requiresPerUserAuth: Bool {
        switch self {
        case .apiKey: false
        case .jellyfinCredentials: true
        case .localAccount: true
        }
    }
}
...
public extension SeerrServer {
    ...
    func isAuthenticatedForCurrentUser() -> Bool {
        switch authMode {
        case .apiKey:
            KeychainManager.existsShared(key: "seerr_apikey_\(id)")
        case .jellyfinCredentials, .localAccount:
            KeychainManager.hasSeerrSession(for: id)
        }
    }
}
